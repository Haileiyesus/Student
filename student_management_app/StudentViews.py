...
from student_management_app.models import CustomUser, Staffs, Courses, Subjects, Students, Attendance, AttendanceReport, LeaveReportStudent, FeedBackStudent, StudentResult,StudentActivate,LoginType
...
def student_view_result(request):
    result = LoginType.objects.get(id=1)
    student = Students.objects.get(admin=request.user.id)
    student_result = StudentResult.objects.filter(student_id=student.id)
    context = {
        "student_result": student_result,
    }
    if (result.result_type == True):
        return render(request, "student_template/student_view_result.html", context)
    else:
        return redirect('student_home')

def student_activate(request):
    activate = LoginType.objects.get(id=1)
    student_obj = Students.objects.get(admin=request.user.id)
    activate_data = StudentActivate.objects.filter(student_id=student_obj)
    context = {
        "activate_data": activate_data
    }
    if (activate.activate_type == True):
        return render(request, 'student_template/student_activate.html', context)
    else:
        return redirect('student_home')
     
    
def student_activate_save(request):
    if request.method != "POST":
        messages.error(request, "Invalid Method!")
        return redirect('student_activate')
    else:
        student_id = Students.objects.get(admin=request.user.id)
        date = request.POST.get('activate_date')
        activater_name = request.POST.get('activater_name')
        activate_birr = request.POST.get('activate_birr')
        activate_txn = request.POST.get('activate_txn')


        try:
            activate = StudentActivate(student_id=student_id, date=date, activater_name=activater_name,activate_birr=activate_birr,activate_txn=activate_txn, activate_status=0)
            activate.save()
            messages.success(request, "You Finshed Successfully!")
            return redirect('student_activate')
        except:
